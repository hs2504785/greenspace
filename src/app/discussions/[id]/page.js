...
export default function DiscussionDetailPage({ params }) {
  ...
  if (discussionLoading) return <div className="container py-4"><div className="alert alert-info">Loading discussion...</div></div>;
  if (discussionError) return <div className="container py-4"><div className="alert alert-danger">{discussionError.message}</div></div>;

  return (
    <div className="container py-4">
      {/* Discussion Header */}
      <div className="d-flex justify-content-between align-items-center mb-3">
        <button className="btn btn-outline-secondary" onClick={() => router.push('/discussions')}>
          ← Back to Discussions
        </button>
      ...
      <div className="card mb-4">
        <div className="card-header">
          ...
          <small className="text-muted">
            Posted {discussion?.created_at ? new Date(discussion.created_at).toLocaleDateString() : ''}
          </small>
        </div>
        <div className="card-body">
          <p className="card-text" style={{ whiteSpace: 'pre-wrap' }}>{discussion?.content}</p>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="card-body">
          {messagesLoading && <div className="alert alert-info">Loading replies...</div>}
          {messagesError && <div className="alert alert-danger">{messagesError.message}</div>}
          
          {messages && messages.data && messages.data.length === 0 && (
            <div className="alert alert-warning">No replies yet. Be the first to reply!</div>
          )}

          {messages && messages.data && messages.data.map((message) => (
            <div key={message.id} className="border-bottom pb-3 mb-3">
              ...
              <p className="mt-2 mb-0" style={{ whiteSpace: 'pre-wrap' }}>{message.content}</p>
            </div>
          ))}
          ...
          {session ? (
            <form onSubmit={handleReplySubmit} className="mt-4">
              <div className="mb-3">
                <label htmlFor="reply" className="form-label">Post a Reply</label>
                <textarea
                  id="reply"
