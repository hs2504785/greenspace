...
class EnhancedVegetableService {
  constructor() {
    this.internalService = VegetableService;
    this.externalService = GoogleSheetsService;
  ...
  async getAllVegetables(options = {}) {
    ...
    try {
      ...
      if (includeExternal) {
        ...
        console.log(`📋 Fetched ${userSheetProducts.length} user sheet products`);
      }

      // Combine and return
      const allProducts = [...internalProducts, ...externalProducts, ...userSheetProducts];
      console.log(
...
export default new EnhancedVegetableService();
